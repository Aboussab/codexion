...
static void creat_coders(simulation* simulater)
{
    int     i;
    int     n;
    ...
    while (i < n)
    {
        ...
        pthread_mutex_init(&simulater->all_coders[i].counter_compiling_mutex, NULL);
        simulater->all_coders[i].right_dongle = &(simulater->all_dongles[(i + 1) % n]);
        i++;
    }
}
static void   creat_dongels(simulation* simulater)
{
   int     i;
