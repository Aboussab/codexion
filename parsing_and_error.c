...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/08/09 17:45:16 by aboussab          #+#    #+#             */
/*   Updated: 2026/08/09 19:09:43 by aboussab         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_arg(char **argv, int argc)
{
	...
	if (argc != 9)
		return (error_join(\
"the number of arg most be 6 number and one scheduler"), 0);
	i = 1;
	while (i <= 7)
	{
	...
	if (strcmp(argv[8], "edf") != 0 && strcmp(argv[8], "fifo") != 0)
		return (error_join(\
"the scheduler arg most be either an 'fifo' or 'edf'"), 0);
	return (1);
}
...
t_parse	*ft_parse(int argc, char **argv)
{
	...
	if (!ft_check_arg(argv, argc))
		return (NULL);
	arg = (t_parse *)malloc(sizeof(t_parse));
	if (!arg)
		return (NULL);
	...
	arg->n_compiles_required = atoi(argv[6]);
